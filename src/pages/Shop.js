...
const Shop = () => {
  ...
  const loadFilteredProducts = async () => {
    try {
      const { data } = await axios.get("https://jsonplaceholder.typicode.com/posts"); // Update the API endpoint with the desired JSONPlaceholder API endpoint
  ...
  const loadCatgories = async () => {
    try {
      const { data } = await axios.get("/categories"); // Use the dummy API endpoint for categories
      setCategories(data);
    } catch (err) {
      console.log(err);
    }
